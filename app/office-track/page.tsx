...
function ReviewContent() {
  ...
  return (
    ...
    <IoIosArrowBack
      size={30}
      className="absolute top-[2%] left-4 sm:top-20 sm:left-6 md:top-20 md:left-8 lg:top-20 lg:left-10 hover:bg-[#BD1521]/20 p-1 rounded-full transition-colors text-[#BD1521]"
      onClick={() => router.push("/office-7")}
    />
    ...
    <div className="py-6">
      <div className="mx-1 flex items-center justify-between">
        <div className="text-center flex-1">
          <h2 className="font-['Noe_Display'] text-[26px] sm:text-[32px] leading-[40px] text-[#BD1521] font-bold m-0">
            {companyName}
          </h2>
        </div>
        <div className="w-[24px]"></div>
    ...
    <div className="max-w-3xl mx-auto my-8 px-4">
      ...
      {selections.map((item, index) => (
        <div key={index} className="w-full max-w-lg mx-auto my-4">
          <div className="flex items-center bg-[#F5F5F5] min-h-[50px] py-3 rounded-lg hover:scale-105 transition-all">
            <div className="w-16 text-center">
              <item.icon className="w-6 h-6 mx-auto text-[#BD1521]" />
            </div>
            <div className="flex-1">
              {Array.isArray(item.label) ? (
                item.label.map((text, i) => (
                  <div key={i} className="font-['Apercu-Medium'] text-[18px] text-[#BD1521] font-bold">
                    {text}
                  </div>
                ))
              ) : (
                <span className="font-['Apercu-Medium'] text-[18px] text-[#BD1521] font-bold">
                  {item.label}
                </span>
              )}
            </div>
      ...
      <div className="max-w-lg mx-auto mb-12 mt-16 px-0 text-center">
        ...
        <div className="flex flex-wrap gap-4 justify-center mb-6">
          {selectedOptions.map((option) => (
            <div
              key={option}
              className="w-full sm:w-[calc(50%-1rem)] bg-[#BD1521] rounded py-4 px-6 border border-white hover:scale-105 transition-all"
            >
              <p className="font-['Apercu'] text-[18px] text-white mb-0 flex items-center justify-center">
                <FaCheck className="w-5 h-5 mr-3" />
                {option.replace("-", " ")}
              </p>
            </div>
